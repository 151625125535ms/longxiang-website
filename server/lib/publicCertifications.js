...
const QUALIFICATION_NAMES = Object.freeze({
...
function publicCertificationName(certification, locale) {
    const localizedNames = {
        ar: certification.name_ar,
        fr: certification.name_fr,
        ru: certification.name_ru
    };
    const sourceName = locale === 'en' ? certification.name_en : localizedNames[locale];
    if (sourceName && !containsCjk(sourceName)) return sourceName;
    if (locale === 'fr' || locale === 'ru') return '';
    ...
    if (QUALIFICATION_NAMES[id]) return QUALIFICATION_NAMES[id];
...
function readPublicCertifications(dbValue) {
...
module.exports = { readPublicCertifications, publicCertificationName };
