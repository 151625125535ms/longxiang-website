...
- `.com` 仅保留页脚中的中国官网普通链接。
- `.cn` 永久不属于本项目的执行范围，不规划 301、canonical、noindex、hreflang、内容改写或站长平台操作。
...
- 19 个英文产品详情页的竞争主题覆盖表：[`stage1-cn-product-competition-monitoring-20260711.md`](stage1-cn-product-competition-monitoring-20260711.md)。
...
这些 URL 使用 `.cn` 的英文/default 路径体系。观察它们的目的仅是识别与 `.com` 的主题竞争和内容缺口，不把 `.cn` 流量、外链或技术配置作为本项目的完成条件。
...
4. `.com` 不依赖 `.cn` 权重转移，而是通过海外差异化内容、关键词、内链、外链和询盘转化独立建立权重。

## 可选的未来公开观察

阶段一不再等待 `.cn` Search Console 数据。后续季度观察只记录公开可见信息：

- sitemap URL 数及页面类型变化；
- 公开搜索结果中的 `.cn` 英文页面样本；
- 与 `.com` 重合的产品主题、标题和搜索意图；
- `.com` 是否已经用独立海外内容覆盖相应主题。
